...
tinymce.PluginManager.add('glyphicon_block', function(editor, url) {
	...
	// add custom css
	editor.contentCSS.push(url+'/font/glyphicons-halflings.css');
	...
	var menu_item = {
		...
		onclick: function() {
			if(blockade.isPlaceable(blockade.body)) {
				...
				el.innerHTML = [
					'<div class="wp-blockade-glyphicon-wrapper wp-blockade-glyphicon-center wp-blockade-glyphicon-3em">',
						'<i class="glyphicon glyphicon-asterisk">',
							'<span class="sr-only">No Label</span>',
						'</i>',
					'</div>'
				].join('');
				blockade.setData(el, blockade.datafields.type, 'glyphicon');
	...
	blockade.contenttypes.glyphicon = {
		name : "Glyphicon",
		parse_block_data : function( data, block ) {
			var wrap = block.querySelector('.wp-blockade-glyphicon-wrapper');
			var glyphicon = wrap.querySelector('.glyphicon');
			data.type_specific = {
			...
			data.type_specific.new_win = !!wrap.getAttribute('target');
			data.type_specific.label = glyphicon.textContent;
			var glyphStyle = glyphicon.getAttribute('style');
			if( glyphStyle ) {
				var styleParts = glyphStyle.replace( ';', '' ).split( ':' );
				if( styleParts && styleParts[1] ) {
					data.type_specific.color = styleParts[1].trim();
				}
			}

			var wrapclasses = wrap.getAttribute('class').split(' ');
			for(var i = 0; i < wrapclasses.length; i++) {
				if( wrapclasses[i] == "wp-blockade-glyphicon-wrapper" ) {
					continue;
				}
				switch( wrapclasses[i] ) {
					case "wp-blockade-glyphicon-left" :
						data.type_specific.align = 'left';
						break;
					case "wp-blockade-glyphicon-right" :
						data.type_specific.align = 'right';
						break;
					case "wp-blockade-glyphicon-1em" :
						data.type_specific.size = '1em';
						break;
					case "wp-blockade-glyphicon-2em" :
						data.type_specific.size = '2em';
						break;
					case "wp-blockade-glyphicon-4em" :
						data.type_specific.size = '4em';
						break;
					case "wp-blockade-glyphicon-5em" :
						data.type_specific.size = '5em';
				}
			}
			var glyphclasses = glyphicon.getAttribute('class').split(' ');
		...
		render_html : function( data ) {
			...
			var str = [
				'<div class="wp-blockade-glyphicon-options">',
					blockade.options_make_select_box_html( 'Icon', 'icon', glyphicon_select, data.type_specific.icon ),
					'<label>',
						'<span>A11y Label: </span>',
						'<input type="text" name="label" class="mce-textbox" value="' + data.type_specific.label + '"/>',
					'</label>',
					blockade.options_make_accordion_html("Options", [
						blockade.options_make_color_picker_html( "Color", 'color', data.type_specific.color ),
					...
					blockade.options_make_accordion_html("Link", [
						...
						'</label>',
						'<label>',
							'<input type="checkbox" name="new_win" value="true"' + checked + '/>',
		...
		apply_form_results : function( data, form_data, block ) {
			var content = '<span class="sr-only">' + form_data.label + '</span>';
			content = '<i class="glyphicon ' + form_data.icon + '" style="color:' + form_data.color + ';">' + content + '</i>';
			var atts = ' class="wp-blockade-glyphicon-wrapper wp-blockade-glyphicon-' + form_data.align + ' wp-blockade-glyphicon-' + form_data.size + '"';
			if( form_data.href ) {
				var target = "";
				if( form_data.new_win ) {
					target = ' target="_blank"';
				}
				content = '<a href="' + form_data.href + '"' + target + atts + '>' + content + '</a>';
			} else {
				content = '<div ' + atts + '>' + content + '</div>';
			}
